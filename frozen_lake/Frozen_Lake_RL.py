...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for curr_state in range(nS):
            snext_value_function = 0
            snext_reward = 0
            given_action = policy [curr_state]
            env_effects =len(P[curr_state][given_action])
            for effect in range(env_effects):
                snext_value_function += (P[curr_state][given_action][effect][0] * prev_value_function[P[curr_state][given_action][effect][1]])    
                snext_reward += (P[curr_state][given_action][effect][0] * P[curr_state][given_action][effect][2])
            value_function [curr_state] = 1 * (snext_reward + gamma * snext_value_function)
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for curr_state in range(nS):
        ...
        for action in P[curr_state]:
            env_effects = len( P[curr_state][action])
            for effect in range(env_effects):
                q_value_function[action] += P[curr_state][action][effect][0] * ( P[curr_state][action][effect][2] + gamma * value_from_policy [ P[curr_state][action][effect][1]])
        q_value = np.argmax(q_value_function)
        if policy[curr_state] == q_value:
           new_policy[curr_state] = policy[curr_state]
        else:
            new_policy[curr_state] = q_value
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    value_function = np.array(value_from_policy)
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        prev_value_function = np.array(value_function)
        for currstate in range(0,nS-1):
            snext_value_function = []
            snext_reward = []
            for action in P[currstate]:
                env_effects =len(P[currstate][action])
                sum_effect_value=0
                sum_effect_reward=0
                for effect in range(env_effects):
                    sum_effect_value+=(P[currstate][action][effect][0] * prev_value_function[P[currstate][action][effect][1]])
                    sum_effect_reward+=(P[currstate][action][effect][0] * P[currstate][action][effect][2])
                snext_value_function.append( gamma * sum_effect_value)
                snext_reward.append(sum_effect_reward)
            maxv=list( np.add( snext_reward, snext_value_function))
            value_function[currstate] = 1 * max(maxv)
            policy[currstate] = maxv.index( max(maxv))
        if (max( abs(value_function - prev_value_function)) < tol):
            break
...
def render_single(env, policy, max_steps=100):
    ...
    for t in range(max_steps):
        ...
        time.sleep(0.25) 
        a = policy[ob]
        ob, rew, done, _,_prob = env.step(a)       # remove a blank after done if on Colab 
        episode_reward += rew
        if done:
            break
    env.render()
